...
def plot_bar(subplt, xvals, yvals0, yvals1, label0, label1, xlabel, ylabel, title):
    ...
    subplt.set_xlabel(xlabel, fontsize=22)
    subplt.set_ylabel(ylabel, fontsize=22)
    subplt.set_title(title, fontsize=26)
    subplt.set_xticks(index + bar_width / 2)
    subplt.yaxis.set_tick_params(labelsize=22)
    subplt.set_xticklabels(xvals, fontsize=22)
    subplt.legend(fontsize=22)
...
create_subplots(sorted_throughputs_tx0, sorted_throughputs_tx1, 'Average Throughput (Mbps)', 'TX0 (Prague)',
                'TX1 (Cubic)')
plt.suptitle('Average Throughput vs Queue Type - ' + 'Buffer = ' + str(specified_params['n_bdp']) + 'BDP-' + 'ECN_thresh = ' + str(
    specified_params['ecn_threshold']) + 'ms -' + 'rx0_ecn:' + str(
    specified_params['rx0_ecn']) + '-' + 'rx1_ecn:' + str(specified_params['rx1_ecn']) + '-' + 'ecnfallback:' + str(
    specified_params['ecn_fallback']), fontsize=32, y=0.99)
plt.tight_layout()
plt.savefig('barplot_throughput.png')
# plt.show()

# Generate subplots for RTT
create_subplots(sorted_rtts_tx0, sorted_rtts_tx1, 'Average SRTT (ms)', 'TX0 (Prague)', 'TX1 (Cubic)')
plt.suptitle('Average SRTT vs Queue Type - ' + 'Buffer = ' + str(specified_params['n_bdp']) + 'BDP-' + 'ECN_thresh = ' + str(
    specified_params['ecn_threshold']) + 'ms -' + 'rx0_ecn:' + str(
    specified_params['rx0_ecn']) + '-' + 'rx1_ecn:' + str(specified_params['rx1_ecn']) + '-' + 'ecnfallback:' + str(
    specified_params['ecn_fallback']), fontsize=32, y=0.99)
plt.tight_layout()
plt.savefig('barplot_srtt.png')
# plt.show()
